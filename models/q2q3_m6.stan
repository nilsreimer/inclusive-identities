...
parameters {
  ...
  vector[N_col] b_obc;
  vector[N_col] b_q1;
  vector[N_col] b_q1_scst;
  vector[N_col] b_q1_obc;
  matrix[N_col, J] b_j_z;  
  matrix[K, N_col] b_k_z;
  matrix[K, N_col] b_q1_k_z;
  matrix[4, N_col] b_scst_k_z; 
  matrix[4, N_col] b_obc_k_z;
  matrix[4, N_col] b_q1_scst_k_z; 
  matrix[4, N_col] b_q1_obc_k_z;
...
transformed parameters {
  ...
  for (m in 1:N_col) {
    for (n in 1:N_row) {
      Y[n, m] = y[(m - 1) * N_row + n];
      Mu[n, m] = b_0[m] + b_j[jj[n], m] + b_k_z[kk[n], m]*sigma_k[m] + x_q1[n]*(b_q1[m] + b_q1_k_z[kk[n], m]*sigma_q1_k[m]);
      if (kk[n] <= 4) {
        Mu[n, m] += x_scst[n]*(b_scst[m] + b_scst_k_z[kk[n], m]*sigma_scst_k[m] + x_q1[n]*(b_q1_scst[m] + b_q1_scst_k_z[kk[n], m]*sigma_q1_scst_k[m])) + x_obc[n]*(b_obc[m] + b_obc_k_z[kk[n], m]*sigma_obc_k[m] + x_q1[n]*(b_q1_obc[m] + b_q1_obc_k_z[kk[n], m]*sigma_q1_obc_k[m]));
      }
    }
  }
}
model {
  ...
  b_obc ~ normal(0, 1);
  b_q1 ~ normal(0, 1);
  b_q1_scst ~ normal(0, 1);
  b_q1_obc ~ normal(0, 1);
  to_vector(b_j_z) ~ normal(0, 1);
  to_vector(b_k_z) ~ normal(0, 1);
  to_vector(b_q1_k_z) ~ normal(0, 1);
  to_vector(b_scst_k_z) ~ normal(0, 1);
  to_vector(b_obc_k_z) ~ normal(0, 1);
  to_vector(b_q1_scst_k_z) ~ normal(0, 1);
  to_vector(b_q1_obc_k_z) ~ normal(0, 1);
  sigma ~ cauchy(0, 1);
  sigma_j ~ cauchy(0, 1);
  sigma_k ~ cauchy(0, 1);
  sigma_q1_k ~ cauchy(0, 1);
  sigma_scst_k ~ cauchy(0, 1);
  sigma_obc_k ~ cauchy(0, 1);
  sigma_q1_scst_k ~ cauchy(0, 1);
  sigma_q1_obc_k ~ cauchy(0, 1);
