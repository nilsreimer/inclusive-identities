...
# Prepare -----------------------------------------------------------------
  ...
  dz <- dw %>%  
    mutate_at(
      vars(starts_with("pd_"), starts_with("x_nc_"), starts_with("x_of_")), 
  ...
  data_list <- with(dz, list(
    ...
    K = 6L,
    y = cbind(pd_ig, pd_scst, pd_obc, pd_gm, pd_hindu, pd_muslim)
  ))
